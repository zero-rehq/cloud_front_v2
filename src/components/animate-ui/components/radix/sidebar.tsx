...
function SidebarProvider({
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    ...
    [state, open, setOpen, isMobile, openMobile, setOpenMobile, toggleSidebar, showPreview],
  );
...
function Sidebar({
  ...
  const { isMobile, state, openMobile, setOpenMobile, showPreview } = useSidebar();
  ...
  return (
    ...
    >
      ...
      {/* Preview overlay when collapsed and trigger is hovered */}
      {state === 'collapsed' && showPreview && !isMobile && (
        <div
          data-slot="sidebar-preview"
          className={cn(
            'fixed inset-y-0 z-40 w-(--sidebar-width) backdrop-blur-md',
            'animate-in fade-in-0 slide-in-from-left-4 duration-200',
            side === 'left' ? 'left-0' : 'right-0',
            'bg-sidebar/98 border-r-2 border-sidebar-border shadow-2xl'
          )}
        >
          <div className="size-full opacity-90 overflow-auto">
            {children}
          </div>
        </div>
      )}
...
function SidebarTrigger({ className, onClick, ...props }: SidebarTriggerProps) {
  const { toggleSidebar, state, setShowPreview } = useSidebar();
  ...
  const handleMouseLeave = React.useCallback(() => {
    ...
    // Delay hiding the preview to allow moving to the sidebar
  ...
  React.useEffect(() => {
    // Detect mouse over the preview area
    const handleGlobalMouseMove = (event: MouseEvent) => {
      if (state !== 'collapsed') return;

      const previewElement = document.querySelector('[data-slot="sidebar-preview"]');
      if (previewElement) {
        ...
        if (isOverPreview) {
          // Cancel hide timeout if mouse is over preview
          if (leaveTimeoutRef.current) {
            clearTimeout(leaveTimeoutRef.current);
            leaveTimeoutRef.current = null;
          }
        } else if (!isOverPreview) {
          // Check if mouse is over trigger button
          const triggerElement = document.querySelector('[data-slot="sidebar-trigger"]');
          if (triggerElement) {
            ...
            if (!isOverTrigger) {
              // Hide preview if not over trigger or preview
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        'hover:scale-110 hover:bg-sidebar-accent/50',
        'active:scale-95',
        'group/trigger',
        className
      )}
    ...
    >
      <PanelLeftIcon
        className={cn(
          ...
          'group-hover/trigger:rotate-12',
          'group-active/trigger:rotate-0'
        )}
      />
...
function SidebarInset({ className, ...props }: SidebarInsetProps) {
  return (
    <main
      ...
      className={cn(
        'bg-default relative flex w-full flex-1 flex-col',
